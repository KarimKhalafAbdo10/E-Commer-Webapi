    ...
    internal class CatalogDataSeeder(StoreDbContext dbContext, ILogger<CatalogDataSeeder> logger) : IDataSeeder
    {
        public async Task SeedDataAsync(CancellationToken ct = default)
        {
            try
            {
                ...
                if (PendingMigrations.Any()) await dbContext.Database.MigrateAsync(ct);
           ...
           int result=    await dbContext.SaveChangesAsync(ct);
            ...
            {
                logger.LogError(ex, "Error while seeding catalog data.");
                throw;
            }

        }

        private async Task SeedIfEmptyAsync<T, Tkey>(string rootPah, string fileName, CancellationToken ct = default) where T : BaseEntity<Tkey>
        {
            ...
            var filePath = Path.Combine(rootPah, fileName);

            if (!File.Exists(filePath))
            {
                
                    logger.LogError($"File {fileName} Is Not Exist");
                    return;
